...
Run each set of tests (specs) as using the `rspec` command from the project's root directory via command line: `rspec test_01.rb`.
...
4. Run the test suite using the same `rspec` command to verify that your code is passing the test you are trying to pass
